...
$stmt = $pdo->prepare('SELECT * FROM pets WHERE id=$1 AND listed_by=$2');
$stmt->execute([$petId, $sellerId]);
$pet = $stmt->fetch();
if (!$pet) { header('Location: dashboard.php'); exit; }

$errors = $_SESSION['errors'] ?? []; unset($_SESSION['errors']);
$old    = $_SESSION['old']    ?? $pet; unset($_SESSION['old']);

function oval($key, $default='') { global $old; return htmlspecialchars($old[$key] ?? $default); }
?>
<!doctype html>
<html lang="en">
<head>
    ...
    <form method="POST" action="save-pet.php" enctype="multipart/form-data" class="pet-form">
        ...
        <div class="form-group">
          ...
          <select name="type" required>
            <?php foreach(['dog','cat','rabbit','bird','other'] as $t): ?>
              <option value="<?=$t?>" <?= oval('type')===$t?'selected':'' ?>><?= ucfirst($t) ?></option>
            <?php endforeach; ?>
          </select>
        </div>
        <div class="form-group">
      ...
      <div class="form-group full">
        ...
        <div style="display:flex;align-items:center;gap:16px;margin-bottom:8px;">
          <img src="../<?= htmlspecialchars($pet['image_path']) ?>" alt="" style="width:80px;height:80px;object-fit:cover;border-radius:8px;"/>
